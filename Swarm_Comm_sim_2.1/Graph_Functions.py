...
def insert_nodes(graph, path_n, path_col, node_gdf, node_col, prev_node_name):
    for i, node in node_gdf.iterrows():
        if node['path_n'] == path_n and graph.has_node(node['name']) == False:
            ...
            d = np.sqrt((graph.nodes[node['name']]['pos'][0] - graph.nodes[prev_node_name]['pos'][0])**2 + (graph.nodes[node['name']]['pos'][1] - graph.nodes[prev_node_name]['pos'][1])**2)
            graph.add_edge(node['name'], prev_node_name, color = path_col, weight = d)
...
def populate_graph(graph, boarder_col, path_gdf, path_col, intersection_gdf, intersection_col, node_gdf, node_col):
    for i, path in path_gdf.iterrows():
        ...
        if path_intrs.shape[0] == 0:
            ...
            d = np.sqrt((graph.nodes[path['end_name']]['pos'][0] - graph.nodes[prev_node_name]['pos'][0])**2 + (graph.nodes[path['end_name']]['pos'][1] - graph.nodes[prev_node_name]['pos'][1])**2)
            graph.add_edge(prev_node_name, path['end_name'], colour = path_col, weight = d)
        else: 
            for j, intr in path_intrs.iterrows():
                ...
                prev_node_name = insert_nodes(graph, path['path_n'], path_col, node_gdf, node_col, prev_node_name)
                d = np.sqrt((graph.nodes[intr['name']]['pos'][0] - graph.nodes[prev_node_name]['pos'][0])**2 + (graph.nodes[intr['name']]['pos'][1] - graph.nodes[prev_node_name]['pos'][1])**2)
                graph.add_edge(prev_node_name, intr['name'], color = path_col, weight = d)
                prev_node_name = intr['name']
            prev_node_name = insert_nodes(graph, path['path_n'], path_col, node_gdf, node_col, prev_node_name)
            d = np.sqrt((graph.nodes[path['end_name']]['pos'][0] - graph.nodes[prev_node_name]['pos'][0])**2 + (graph.nodes[path['end_name']]['pos'][1] - graph.nodes[prev_node_name]['pos'][1])**2)
            graph.add_edge(prev_node_name, path['end_name'], color = path_col, weight = d)